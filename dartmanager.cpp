...
void DartManager::moveDart()
{
    ...
    {
        // 检测出界
        if ( step->position.x() < 0 || step->position.x() > sm->wh_ratio ||
             step->position.y() < 0 || step->position.y() > 1 )
        {
...
bool DartManager::checkCollision( QVector2D point, double width, double height )
{
    double temp_x;
    double temp_y;
    for ( std::vector<Dart>::iterator step = dart_arr.begin();
